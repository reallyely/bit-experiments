...
export class WikiRoutes {
  constructor(readonly tree: WikiRoute[], readonly basePath?: string) {}
  ...
  toSideBarTree() {
    return {
      id: '',
      children: this.tree.map((node) =>
        this.computeTreeNode(node, this.basePath)
      ),
    };
  }
  ...
  private computePayload(docRoute: WikiRoute, parentPath?: string) {
    // basePath should be included in the parent path
    const thisPath = this.accumulatePath(docRoute.path, parentPath).replace(
      starRegex,
      ''
    );
    ...
    return {
      ...
      configPath:
        docRoute.config?.path &&
        this.accumulatePath(docRoute.config.path, thisPath),
      overviewPath:
        docRoute.overview?.path &&
        this.accumulatePath(docRoute.overview.path, thisPath),
      path: thisPath,
  ...
  private computeTreeNode(treeNode: WikiRoute, parentPath?: string) {
    if (treeNode.children) {
      return {
        id: treeNode.path,
        children: treeNode.children.map((child) =>
          this.computeTreeNode(
            child,
            this.accumulatePath(treeNode.path, parentPath)
          )
        ),
  ...
  private computeRoutes(currentRoute: WikiRoute, parentPath?: string): Route[] {
    ...
    if (currentRoute.children) {
        ...
        ? [
            {
              ...
              absPath: this.toAbs(
                this.accumulatePath(currentRoute.path, thisPath)
              ),
      ...
      return flatten(
        currentRoute.children.map((child) =>
          this.computeRoutes(child, thisPath)
        )
      )
        .concat(configRoutes)
  ...
  static from(routes: WikiRoute[], basePath: string = '/') {
    return new WikiRoutes(routes, basePath);
  }
}
